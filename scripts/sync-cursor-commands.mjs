 ...
 * as literal text). This is the same transform `create-blit386`'s
 * `@blit386/kit` applies when scaffolding a Cursor-flavored project - see
 * `generateCursorAdapter` in that repo's `packages/kit/src/adapters.ts`. Any
 * `../`-prefixed link target is also re-relativized: `.cursor/commands/` sits
 * one directory level shallower than `.claude/skills/<name>/`, so a straight
 * copy of a link like `../../../docs/security/security-runbook.md` would
 * point one level above the repo root.
 *
