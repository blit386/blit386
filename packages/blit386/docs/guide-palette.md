...
`paletteFadeExposure` does the multiplication in linear light. Worth recording so nobody re-derives it wrong later:
fading in linear light does not by itself make highlights linger relative to shadows. sRGB is close to a pure power
curve, and a power curve is scale-invariant – multiplying linear light by `k` multiplies every encoded value by roughly
the same factor. You get a better-shaped curve, not a differential between white and dark gray.
