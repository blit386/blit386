...
Triggers on push to `main` and on pull requests targeting `main`. `labeled` / `unlabeled` PR events skip every job
except `quality-engine`, and that one only when the added label is `perf` (which enables the benchmark job).
