...
| D10 | End-user package manager | Auto-detect; npm default | Scaffolder detects the manager that invoked it (`npm_config_user_agent`, like create-vite) and uses it for install + lockfile + documented run commands. Generated `package.json` also gets a Corepack `packageManager` (`name@version`) from that agent so Corepack does not auto-add one on first install. Docs default to npm (ships with Node; zero extra install for kids). pnpm-only remains Václav's own-repo rule, not forced on end users. |
