...
| D4 | Default UX | Short wizard | No silent default. 2-3 questions (JS or TS, which assistants - multi-select - or none). Kid-friendly copy for the unsure. Pros pass flags to skip. |
...
| D10 | End-user package manager | Auto-detect; npm default | Scaffolder detects the manager that invoked it (`npm_config_user_agent`, like create-vite) and uses it for install + lockfile + documented run commands. Generated `package.json` gets a Corepack `packageManager` (`name@version`) from that same agent string, using the full exact semver (prerelease and build suffixes included). Scaffolding stops when the string has no exact version, so Corepack does not auto-add a different pin on first install. Bun scaffolds omit the field: Corepack accepts only npm, pnpm, and yarn. Docs default to npm (ships with Node; zero extra install for kids). pnpm-only remains Václav's own-repo rule, not forced on end users. |
