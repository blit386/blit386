...
test('migrationsThrough includes the 1.x rename migration and excludes future ones', () => {
    ...
    assert.equal(migrationsThrough('1.1.0').length, 2, 'both rename migrations apply from 1.1.0');
    assert.equal(migrationsThrough('1.4.0').length, 3, 'hot-reload vite migration applies from 1.4.0');
    assert.equal(migrationsThrough('1.5.0').length, 3, 'all three still apply on a newer 1.x');
...
test('applyRenames rewrites importPath specifiers without a leading dot', () => {
    const importPathRename = [{ from: 'blit386', to: 'blit386', kind: 'importPath', safety: 'auto' }];

    const esm = "import { BT } from 'blit386';";
    const cjs = "const bt = require('blit386');";
    const scoped = "import { BT } from 'blit386-extra';";
