...
- The `blit` CLI - a small helper you run inside a BLIT386 game:
  ...
  - `blit agents sync` - refresh the AI-assistant files from the installed kit. It keeps your edits: kit-owned files you
    ...
    saves the kit version as `<file>.new`. Use `--check` to report drift without writing (CI-safe; `blit doctor` runs it
    too), or `--force [path...]` to take the kit version back. Once sync has merged your edits into a kit file,
    `--check` treats that file as settled - it will not keep reporting it as drifted.
