...
lastModified: "2026-09-25T13:41:33+02:00"
...
2. The `?splash` and `?nosplash` URL flags. Both are valueless. (The engine's other URL parameters live elsewhere:
   `?backend=software` in [API: Core](/docs/api/core#requested-vs-active-backend), `?reducedmotion` / `?noreducedmotion`
   under [Reduced motion](#reduced-motion), and `?seed=N` in [Random](/docs/guides/random#seeding-from-the-url).)
