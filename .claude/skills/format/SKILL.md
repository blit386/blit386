...
description:
  ...
  package argument (blit386, demos, docs-site, kit, create-blit386, or root for repo-wide files).
---
...
Where `<package>` is one of `blit386`, `demos`, `docs-site`, `kit`, `create-blit386`, or `root` (files outside every
...
- Whole repo (no argument, or `root`): `pnpm run format` from the repository root – Biome across every package (`.ts`,
  ...
  `.yml`, `.yaml`) repo-wide.
- A single package: `pnpm --filter <name> run format` (`blit386`, `blit386-demos`, `blit386-dev-fumapress`, or the
  package's own `name` field), or `cd packages/<dir> && pnpm run format`. `kit` and `create-blit386` have no
  package-local `format` script yet – their files are covered by the root-wide run above.
