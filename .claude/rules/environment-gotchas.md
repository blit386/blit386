...
- `pnpm run spellcheck` scopes to `src/`, `docs/`, and `README.md`, so it does not scan `.claude/skills/`. The
  lint-staged pre-commit `cspell` does scan it, so staging a skill or rule file can surface a pre-existing unknown word;
