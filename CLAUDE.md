...
- Formatting: Biome (TS/JS) + Prettier (MD/YAML). Markdown tables are compact by design (single-space padding, never
  aligned) via `scripts/prettier-plugin-compact-tables.mjs` – this repo holds the canonical copy that the sibling repos
  and the scaffolder template mirror. Do not re-align a table by hand
