...
pnpm run preflight          # All checks (format + lint + typecheck + spellcheck + knip + docs:links + agents:check + sync:doc-banners:check + api:since:check + api:history:check + test:unit + test:declarations + test:agent-config)
```
